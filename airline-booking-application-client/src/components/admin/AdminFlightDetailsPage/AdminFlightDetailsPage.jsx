...
const AdminFlightDetailsPage = () => {
  ...
  const fetchFlightDetails = async () => {
    try {
      ...
      setBookings(
        bookingsRes.data.filter((b) => b.flight?.id === parseInt(id))
      );
    } catch (error) {
      showError(
  ...
  if (loading)
    return (
      <div className="admin-flight-loading">Loading flight details...</div>
    );
  if (!flight)
    return <div className="admin-flight-error">Flight not found</div>;
